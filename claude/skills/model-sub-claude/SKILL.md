...
- Nesting: plain-`Agent` subagents can spawn subagents of their own, and project instructions re-injected into them would pull nested spawns back to the AGENTS.md default. If the chosen model is not `fable`, append one line to every plain-`Agent` subagent prompt: "Default any subagents you spawn to model <model> as well." If it is `fable`, no line is needed — and stop adding one to new spawns if a previous mode required it (agents already running keep the instructions they were given). Workflow-spawned agents have no agent-spawning tools, so they never need the line.
...
- The mode lasts until the session ends or the user switches modes — `/model-sub-claude` (any argument combination) and `/model-sub-codex` form one toggle group; the most recent invocation wins. `/model-sub-claude fable max` (or bare-effort `/model-sub-claude fable`) restores the AGENTS.md defaults.
- Claude Code only: codex never loads this skill and keeps its AGENTS.md default (`gpt-5.5`+`xhigh`).
